...
| ------------------------------------------------- | ------------------------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **api** ([`.env.example`](apps/api/.env.example)) | `DATABASE_URL`                                                                 | **always**             | Non-empty string. Production rejects `sslmode=disable`; the API uses `verify-full` so boot fails closed instead of permitting an unverified connection. |
...
|                                                   | `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`                                    | optional               | Enables Google sign-in when both are set. `GOOGLE_REDIRECT_URI` defaults to `{API_PUBLIC_URL}/oauth/callback/google`.                                   |
...
3. Set environment variables (see `apps/api/.env.example`):
   ...
   - `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` — optional Google OAuth credentials. When both are unset, sign-in remains available through email magic links. `GOOGLE_REDIRECT_URI` defaults to `${API_PUBLIC_URL}/oauth/callback/google`.
   ...
   - `SMTP_HOST` / `SMTP_USER` / `SMTP_PASS` — SMTP credentials for magic-link login. Required when auth is enabled. For Resend use `smtp.resend.com`, port `587`, user `resend`, and a Resend API key as the password. `SMTP_PORT` and `SMTP_FROM` have defaults.
   - `NODE_EXTRA_CA_CERTS` — set to `/app/apps/api/certs/supabase-prod-ca-2021.crt` when `DATABASE_URL` uses Supabase. This keeps hostname and certificate verification enabled for the pooler.
...
| Boot fails with `SELF_SIGNED_CERT_IN_CHAIN`         | Supabase CA is not in Node's default trust store         | Railway deploy logs                               | Set `NODE_EXTRA_CA_CERTS=/app/apps/api/certs/supabase-prod-ca-2021.crt` and redeploy; do not disable certificate verification.                   |
